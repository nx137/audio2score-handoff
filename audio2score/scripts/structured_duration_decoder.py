...
def _rule_v5_enabled() -> bool:
    """v5 记谱网格先验：0.25 拍整数倍候选统一 +0.20（默认关闭）。

    v9 端到端 + 4672 事件排序层模拟双重证伪：
    * 前提错误——参考时值仅 90.3% 落在 0.125 拍网格（32 分音符，含全部
      0.25 整数倍），4.2% 为三连音（1/3 拍）、5.5% 为其他任意值；
      "99.9% 落 0.25 整数倍"的早期分析把 0.125 误并入 0.25 网格。
    * 加性 +0.20 让 0.25 网格候选压过合法的 0.125 网格值（如 0.125->0.25）：
      精确命中 exact 2278->1901（-377）、tol05 0.551->0.414（-640），
      94% 的翻转是选长，直接损害踏板 stop F1（端到端 0.207->0.154）。
    * tol25 的 +270 提升主要是 0.125->0.25 的容差假象（exact 同步 -377）。
    A2S_RULE_V5=1 显式开启仅供复现该负结果；默认回退 v3 公式。
    """
