...
from p4_multivoice_score import decode_midi_to_voice_events
...
def p4_notation_input_notes(midi_path: str, divisors=(8, 4, 3), max_voices=6,
                            feature_scorer=None) -> list:
    """复走 P4 生产解码，返回 MusicXML 导出前的最终符号事件。

    P4 的候选时值有意不同于 M0/P3 的 ``clip_to_next_onset`` 基准；生产回归
    应验证导出层未增删或改变 P4 已选事件，而非把方法本身的时值决策误报为 XML
    误差。若生产导出使用了候选模型，必须传入同一个 ``feature_scorer`` 重现
    完全相同的结构化解码配置。
    """
    decoded, _pedals, _meta, _time_sig, _key = decode_midi_to_voice_events(
        midi_path,
        divisors=divisors,
    ...
    return [
        (event.pitch, event.start_ql, event.duration_ql)
        for hand_events in decoded.values()
...
def main():
    ...
    ap.add_argument("--reference", choices=("notation", "p4", "raw"), default="notation",
                    help="对账基准: notation=P3中间事件(默认),p4=P4解码事件,raw=原始演奏 MIDI")
    ap.add_argument("--max-voices", type=int, default=6,
                    help="P4 对账时的最大声部数，须与生产导出一致")
    ap.add_argument(
        "--candidate-model", default=None,
        help="可选 P4 LightGBM 候选模型前缀；须与生产导出命令一致",
    )
    ...
    if args.candidate_model:
        ...
        if getattr(scorer, "score_features", None) is None:
            raise RuntimeError(
                f"P4 对账要求候选模型可用，无法加载：{args.candidate_model}"
            )
        feature_scorer = scorer
    if args.reference == "notation":
        midi_evts = notation_input_notes(args.midi, divisors=divisors)
        ref_name = "P3 记谱输入事件"
    elif args.reference == "p4":
        midi_evts = p4_notation_input_notes(
            args.midi,
            divisors=divisors,
            max_voices=args.max_voices,
            feature_scorer=feature_scorer,
        )
        ref_name = "P4 解码后的记谱输入事件"
    else:
