#!/usr/bin/env python3
"""C 阶段：冻结 ASAP test 集上 P4 有/无踏板配对消融评测。
...
PIPELINES = ("P4-R", "P4-R-NP", "P4-L", "P4-L-NP")
...
def export_command(pipeline: str, midi_path: Path, xml_path: Path, title: str,
    ...
    if pipeline in ("P4-L", "P4-L-NP"):
        command.extend(["--candidate-model", str(candidate_model)])
    if pipeline.endswith("-NP"):
        command.append("--no-pedal")
    return command
...
def command_log_evidence(commands_log_text: str, xml_path: Path) -> str | None:
    """返回某次导出命令在 commands.log 中对应的完整审计片段。"""
    ...
    return commands_log_text[command_index: next_command if next_command >= 0 else None]


def p4l_load_is_logged(commands_log_text: str, xml_path: Path) -> bool:
    """确认该 P4-L 导出在审计日志中明确报告加载了冻结 LightGBM 模型。"""
    evidence = command_log_evidence(commands_log_text, xml_path)
    return (evidence is not None and "评分=LightGBM：" in evidence
            and "候选模型不可用，回退规则评分" not in evidence)


def no_pedal_is_audited(commands_log_text: str, xml_path: Path) -> bool:
    """确认无踏板导出已在命令审计中报告从解码到符号输出均禁用 CC64。"""
    evidence = command_log_evidence(commands_log_text, xml_path)
    return evidence is not None and "踏板=禁用（不参与时值候选/评分特征/符号输出）" in evidence
...
def main() -> int:
    ...
    if args.resume:
        ...
        expected_config = {
            "schema_version": "C-stage-pedal-ablation-v1",
        ...
        for metrics_path in sorted((run_dir / "pieces").glob("*/*/metrics.json")):
            ...
            if record.get("status") == "completed":
                ...
                if record.get("pipeline") in ("P4-L", "P4-L-NP") and not p4l_load_is_logged(
                        commands_log_text, xml_path):
                    parser.error(f"--resume 无法在 commands.log 中确认 P4-L 已加载 LightGBM：{metrics_path}")
                if record.get("pipeline", "").endswith("-NP"):
                    if not no_pedal_is_audited(commands_log_text, xml_path):
                        parser.error(f"--resume 无法在 commands.log 中确认无踏板审计：{metrics_path}")
                    try:
                        if ET.parse(xml_path).getroot().findall(".//pedal"):
                            parser.error(f"--resume 无踏板 XML 仍含 pedal 元素：{metrics_path}")
                    except ET.ParseError as exc:
                        parser.error(f"--resume 无法解析无踏板 XML：{metrics_path}: {exc}")
    ...
    for row in test_rows:
        ...
        for pipeline in PIPELINES:
            ...
            command = export_command(pipeline, midi_path, xml_path, title, args.divisors,
                                     args.max_voices, model_prefix if pipeline in ("P4-L", "P4-L-NP") else None, scripts_dir)
            export_result = run_command(command, commands_log)
            metrics_record = {
                "schema_version": "C-stage-pedal-ablation-piece-v1",
                "piece_key": piece_key,
            ...
            if pipeline in ("P4-L", "P4-L-NP") and (
                ...
                continue
            if pipeline.endswith("-NP") and "踏板=禁用（不参与时值候选/评分特征/符号输出）" not in export_result.stdout:
                metrics_record["status"] = "no_pedal_not_audited"
                metrics_record["export_stdout"] = export_result.stdout
                metrics_record["export_stderr"] = export_result.stderr
                write_json(out_dir / "metrics.json", metrics_record)
                failures.append({"piece_key": piece_key, "midi_performance": row["midi_performance"],
                                 "pipeline": pipeline, "reason": "无踏板导出未确认禁用 CC64"})
                continue
            ...
            if pipeline.endswith("-NP"):
                try:
                    no_pedal_root = ET.parse(xml_path).getroot()
                except Exception as exc:
                    metrics_record["status"] = "no_pedal_xml_unreadable"
                    metrics_record["scoring_error"] = str(exc)
                    write_json(out_dir / "metrics.json", metrics_record)
                    failures.append({"piece_key": piece_key, "midi_performance": row["midi_performance"],
                                     "pipeline": pipeline, "reason": f"无踏板 XML 无法解析：{exc}"})
                    continue
                if no_pedal_root.findall(".//pedal"):
                    metrics_record["status"] = "no_pedal_xml_contains_pedal"
                    write_json(out_dir / "metrics.json", metrics_record)
                    failures.append({"piece_key": piece_key, "midi_performance": row["midi_performance"],
                                     "pipeline": pipeline, "reason": "无踏板 XML 仍含 pedal 元素"})
                    continue
            try:
    ...
    bootstrap_ci = []
    pipeline_pairs = (("P4-R", "P4-R-NP"), ("P4-L", "P4-L-NP"), ("P4-L", "P4-R"))
    ...
    aggregate_summary = {
        "schema_version": "C-stage-pedal-ablation-summary-v1",
        ...
        "pedal_ablation_definition": {
            "pedal_aware": "P4 解码使用 MIDI CC64 计算声学时值、候选特征并输出 MusicXML pedal。",
            "no_pedal": "P4 解码传入空 CC64 区间；真实 CC64 不参与声学时值、候选特征或排序，也不输出 MusicXML pedal。",
            "paired_primary_comparisons": ["P4-R - P4-R-NP", "P4-L - P4-L-NP"],
        },
        "interpretation_boundaries": [
            "本运行是冻结 ASAP test 上 P4 踏板信息消融；相同 MIDI、量化、手部分配、声部上限、参考谱、外部对齐、容差和统计方法均固定。",
            "P4-R 与 P4-R-NP 是规则评分下的主配对对照；P4-L 与 P4-L-NP 是冻结 LightGBM 评分下的补充配对对照。",
            "无踏板条件在导出日志中必须确认 CC64 禁用，且 MusicXML 不得包含 pedal 元素；任一审计失败均作为运行失败。",
            "本表报告最终 MusicXML 相对参考谱的符号一致性；不替代候选级排序质量或 B 阶段导出忠实性。",
        ],
    }
